...
dist = {
    ...
    makedist('Uniform','lower',lb(4),'upper',ub(4));
    };
%% Implementation
...
for ix = 1:ins
    ...
    for k = 1:m
        ...
        for j = 1:n
            ...
            %calculate s_i
            [t1 ode1]=ode45(@g,[0 .2], xx, [],u(ix));
            [t2 ode2]= ode45(@g,[0 .2], yy, [],u(ix));
            %g(xx,u(ix))
            %g(yy,u(ix))
    ...
    params = mle(l,'pdf',f,'Start',[1 1 max(l)],'Options',opt,'LowerBound',[0 0 max(l)],'UpperBound',[Inf Inf Inf]);
...
dxdt(1) = -1/(2*Heq)*x(1) + 1/(2*Heq)*x(4) + (pav*Nev)/(2*Heq)*u(1) + loss/(2*Heq);
dxdt(2) = -1/(Tg*Req)*x(1) - (1/Tg)*x(2);
dxdt(3) = -T1/(T2*Tg*Req)*x(1) + (Tg-T1)/(T2*Tg)*x(2) -(1/T2)*x(3);
dxdt(4) = (1/Tt)*x(3) - (1/Tt)*x(4);
