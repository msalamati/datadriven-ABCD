...
const double samples = 1807;
const double bias = 3.057e-5;
...
auto radius_post = [](state_type &r, input_type &u) -> void {
    ...
    try {
        ...
        r[0] = m11.get(GRB_DoubleAttr_X)*h/2   + m12.get(GRB_DoubleAttr_X)*h/2+bias;
        r[1] = m21.get(GRB_DoubleAttr_X)*h/2   + m22.get(GRB_DoubleAttr_X)*h/2+bias;
        //}
    }
