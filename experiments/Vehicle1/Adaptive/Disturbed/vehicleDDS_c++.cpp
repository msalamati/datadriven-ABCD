...
double const samples = 4277;
double const bias = 0.044484;
double h_eta = 1.6;
/* sampling time */
...
auto  sample_post = [](state_type &x, input_type &u, input_type &w) -> void {
    ...
    /* the ode describing the vehicle */
    auto rhs =[](state_type& xx,  const state_type &x, input_type &u, input_type &w) {
        double alpha=std::atan(std::tan(u[1])/2.0);
        xx[0] = u[0]*std::cos(alpha+x[2])/std::cos(alpha) + w[0];
        xx[1] = u[0]*std::sin(alpha+x[2])/std::cos(alpha);
        xx[2] = u[0]*std::tan(u[1]);
    };
    ode_solver(rhs,x,u,w);
};
...
auto radius_post = [](state_type &r, input_type &u) {
    if(global_flag == 0){
		...
		randO[0] = random;
            ...
            map.push_front({h_eta,r[0],r[1],r[2],u[0],u[1],m11.get(GRB_DoubleAttr_X), m12.get(GRB_DoubleAttr_X),m13.get(GRB_DoubleAttr_X),m21.get(GRB_DoubleAttr_X), m22.get(GRB_DoubleAttr_X),m23.get(GRB_DoubleAttr_X),m31.get(GRB_DoubleAttr_X),m32.get(GRB_DoubleAttr_X),w1.get(GRB_DoubleAttr_X),w2.get(GRB_DoubleAttr_X),m31.get(GRB_DoubleAttr_X),w3.get(GRB_DoubleAttr_X)});
